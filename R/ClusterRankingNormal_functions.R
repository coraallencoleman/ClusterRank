...
#comes in as y, se, unknown mu, p_mu. We assume se is known here.
#mu i hat = see pics
npmle.norm <- function(y, se, n, k=NULL,n.iter=1000,row_names=NULL) {
  if (is.null(k)) {
    mu<-sort(y)
    k<-length(mu) #k = # groups
  } else {
    mu <- seq(min(y),max(y),length=k)
  }
  p_mu <- seq(min(y),max(y),length=k)
  ...
  for (i in 1:length(mu)) {
    E_z[,i] <- log(p_mu[i])+dnorm(y, mu[i], se, log=TRUE)
  }
  E_z <- t(apply(E_z,1,function(x) exp(x-max(x))/sum(exp(x-max(x)))))
...
rank_cluster.norm <- function(y,sd,k=NULL,scale=identity,weighted=TRUE,n.iter=1000,n.samp=10000,row_names=NULL) {
  N <- length(y)

  npmle_res <- npmle.norm(y,sd,k,n.iter,row_names)

  smp <- apply(npmle_res$post_mu,1,
               function(x,mu,n.samp)
                 sample(mu,n.samp,replace=TRUE,prob=x),
               mu=scale(npmle_res$mu),n.samp=n.samp)
  smp <- t(smp)
  ...
  CI <- normconf(y,n) #TODO find new version of this function
