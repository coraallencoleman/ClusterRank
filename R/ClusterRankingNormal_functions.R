...
npmle.norm <- function(mean, sd, n, k=NULL,n.iter=1000,row_names=NULL) {
  if (is.null(k)) {
    mu<-sort(mean) #sorted means. TODO why do we sort here?
    sd <- sd
    #sd <-sd(mu) #TODO is this who we should calculate this? is this in the correct order?
    k<-length(mu) #k = number of units to rank
  } else {
    mu <- seq(min(mean),max(mean),length=k)
  }
  p_mu <- seq(min(mean),max(mean),length=k) #TODO how to do this for normal? is this to make evenly spaced means for groupings?
  
  E_z <- matrix(NA,length(mean),k) 
  for (j in 1:n.iter) {
    for (i in 1:k) {
      #TODO check this
      E_z[,i] <- log(p_mu[i])+dnorm(mean, mu[i], sd[i], log=TRUE) #TODO E_z is log(pr associated with group) + log(quantile)?
    }
    E_z <- t(apply(E_z,1,function(x) exp(x-max(x))/sum(exp(x-max(x))))) #TODO E_z will be 
    p_mu <- apply(E_z,2,mean)
    mu <- mean%*%E_z/n%*%E_z #TODO what does this do?
  }
