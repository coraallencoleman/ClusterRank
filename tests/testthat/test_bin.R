...
test_that("weighted ranking works", {
  ...
  expect_equal(as.vector(weight$ranked_table$name), c("Middlesex", "Litchfield", "Tolland", "Windham", "New London", "Fairfield", "New Haven", "Hartford"))
})

test_that("unweighted ranking on rank scale works", {
  unweight_rank <- ClusterRank(binData$lbw,binData$births,datatype="binomial",row_names=binData$county, scale=rank, weighted=FALSE)
...
test_that("weighted ranking on rank scale works", {
  weight_rank <- ClusterRank(binData$lbw,binData$births,datatype="binomial",row_names=binData$county, scale=rank, weighted=TRUE)
